...
namespace ReportPortal.E2E.API.Tests.Scenarios.NunitTest
{
    ...
    public class LaunchesQueryByAdminNunitTests : BaseNunitTest
    {
        ...
        {
            var launchesList = Steps.AsAdminUser().GetLaunchesByFilter<GetLaunchesResponse>(ProjectName, data.Query).Launches;
        ...
        {
            var launchesList = Steps.AsAdminUser().GetLaunchesByFilter<GetLaunchesResponse>(ProjectName, query).Launches.OrderBy(x => x.EndTime);
